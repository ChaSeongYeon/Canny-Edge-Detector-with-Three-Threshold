...
# 5. Hysteresis Thresholding(Double)
...
class cannyEdgeDetector:
    def __init__(self, imgs, sigma=1, kernel_size=5, weak_pixel=75, strong_pixel=255, lowthreshold=0.05, highthreshold=0.15):
        ...
        self.nonMaxImg = None # Nonmax Suppresion 적용한 이미지
    ...
    def threshold(self, img):
        ...
        # weak edge 찾기
        # lowThreshold < gradient_magnitude <= highThreshold
    ...
    # strong edge 주변의 weak edge 찾기
    # 위의 조건을 만족하는 weak edge는 strong edge라고 간주
    ...
    def detect(self):
        ...
        for i, img in enumerate(self.imgs):    
            ...
            #print("sobel filter" + str(i))
            self.nonMaxImg = self.non_max_suppression(self.gradientMat, self.thetaMat)
            #print("nonmax suppression" + str(i))
...
def convolution(image, kernel):
    ...
    for i in range(img_h):
        for j in range(img_w):
            output[i, j] = np.sum(padding[i:i + kernel_size, j:j + kernel_size] * kernel_r2)
...
def process_imgs(input_folder, output_folder):
    ...
    # Canny Edfg Detector 클래스 인스턴스 선언
    ...
    for idx, edge_img in enumerate(edges):
        ...
        cv2.imwrite(output_path, edge_img) # Cany Edge Detection 수행한 후의 이미지 저장
